 ...
 <div class="container-fluid">
               ...
               <div class="d-sm-flex align-items-center justify-content-between  ">
                       <div class="global-flex-new1">
                        <h1 class="h3 mb-0 text-gray-800 head1"><i class="bi bi-receipt"></i> All Sales</h1>
                         <p style="color: #adb5bd;">(Total Profit This Month: {{$totalProfit}})</p>
                       </div>
     ...
     @if(Session::has('success'))
               ...
               @endif
                        ...
                        <a href=" {{route('salesdashboard')}}" class=" d-sm-inline-block btn   btn-primary shadow-sm"><i
                                class="bi bi-receipt"></i> Make New Sale</a>
                    </div>
                     <!-- Content Row -->
               ...
               @forelse($record as $part)
                ...
                <tr>
                  <td>{{ $part->id }}</td>
                   ...
                   <td>{{ $part->product->products->name }}</td>
                   <td>{{  $part->quantity}}</td>
                   <td>{{ $part->prod_id}}</td>
                   <td>{{ $part->profit}}</td>
                  
                  <td><div class="global-flex-new justify-content-center"> 
                    <a href="{{ route('rollback', ['id' => $part->id]) }}"><button class="btn btn-secondary btn-sm"> Roll Back</button></a>
                    <a href="{{ route('delete-sale', ['id' => $part->id]) }}"><button class="btn btn-secondary btn-sm"><i class="bi bi-trash"></i></button></a></div></td>
