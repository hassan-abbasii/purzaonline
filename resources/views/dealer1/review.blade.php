...
@endfor &nbsp; {{$record->date}}</p>
        <p style="color: black" class="comment"> {{$record->comment}}</p>
         @if ( $record->reply !== "")
        <p  class="comment comment1" >Reply: {{$record->reply}}</p>
        @endif
        <a href="{{route('replyreviewdealer1',['id' => $record->id])}}">
...
<script>
    ...
    input.addEventListener("input", function() {
        ...
        for (var i = 0; i < reviews.length; i++) {
            ...
            var comment1 = review.getElementsByClassName("comment1")[0].textContent.toLowerCase();

            if (shopName.includes(filter) || comment.includes(filter) || datee.includes(filter) || comment1.includes(filter)) {
