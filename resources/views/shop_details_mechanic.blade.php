...
<div class="container-fluid mechanic">
	<div class="row mechanic_detail">
		<div class="col-md-4  mechanic_detail1">
				...
				<h2>{{$shop->name}}</h2>
				<p>Mechanic&nbsp;<i class="bi bi-patch-check-fill"></i></p>
			</div>
...
@endif
		...
		@endif
			...
			<p class="pca mt-2">Confirm Your Appointment Now...</p>
			<div class="global-flex justify-content-center">

				 
     <a href="{{route('appointment',['id' => $shop->id])}}"><button  class="btn btn-outline-primary "  >Book Appointment</button> </a>
 	

			</div>
			 
				 
				<h4 class="mt-3 service-offer" >Services Offering</h4>
			 
					<div class=" service-details"> 
					@foreach($mechanicServices as $mechanicService)
    <p class="mx-2 px-2">{{ $mechanicService->service }}-{{ $mechanicService->service_category }}</p>
@endforeach
			</div>	 
				 
		 
			<div class="global-flex justify-content-around align-items-baseline">
			<button class="btn btn-primary  btn-sm mt-3" id="replyrp" @if(!session()->has('name')) disabled @endif>Write a Review&nbsp;<i class="bi bi-chat"></i></button>
