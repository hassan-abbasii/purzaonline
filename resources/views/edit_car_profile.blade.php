 ...
 <div class="container-fluid">
   <div class="d-sm-flex align-items-center justify-content-between mb-4">
    <div class="global-flex-new1">
                        <h1 class="h3 mb-0 text-gray-800 head1"><i class="bi bi-person-fill-gear"></i> Edit Car Profile </h1>
                         <p style="color: #adb5bd;">(Provide Relevant Details To Update)</p>
                       </div>
                       <div>
...
<div class="col-md-4 mt-2 mx-2">
  ...
  <h2>Edit  Record</h2>
  <div class="aboutdiv-heading-border4"></div>
  <form method="post" action="{{ route('update-carprofile',['id' => $product->id])}}" id="input-form">
     @error('name')
...
@enderror
 ...
 <div class="form-group">
  <label>Edit Service Date</label>
  <input type="date" name="date1" class="form-control w-75" id="alphabetInput" value="{{ $product->service_date }}" required> 
</div>
  <div class="form-group">
  <label>Edit Service</label>
  <select name="service_id" class="form-control w-75">
    @foreach($services as $car)
      <option value="{{ $car->id }}" {{ $car->id == $product->service_id ? 'selected' : '' }}>
        {{ $car->name }}
      </option>
    @endforeach
  </select>
  ...
  <div class="form-group">
  <label>Edit Current Mileage</label>
  <input type="number" name="mileage" id="mileage" class="form-control w-75" id="alphabetInput" value="{{ $product->mileage }}" required> 
</div>
<div class="form-group">
  <label>Edit Service Cost</label>
  <input type="number" name="cost" id="cost" class="form-control w-75" id="alphabetInput" value="{{ $product->cost }}" required> 
</div>
<div class="form-group">
  <label>Edit Next Service Date</label>
  <input type="date" name="date2" class="form-control w-75" id="date2" value="{{ $product->next_service }}" required> 
</div>
