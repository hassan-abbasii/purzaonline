 ...
 <div class="container-fluid">
   <div class="d-sm-flex align-items-center justify-content-between mb-4">
    <div class="global-flex-new1">
                        <h1 class="h3 mb-0 text-gray-800 head1"><i class="bi bi-tools"></i> Edit Service</h1>
                         <p style="color: #adb5bd;">(Enter Service Details To Update Record)</p>
                       </div>
                       <div>
...
<div class="col-md-4 mt-2 mx-2">
  ...
  <h2>Edit Service</h2>
  <div class="aboutdiv-heading-border4"></div>
  <form method="post" action="{{route('serviceupdate',$services->id)}}" id="input-form">
...
@enderror
  <div class="form-group">
    <label>Select Car Engine</label>
     ...
     @foreach($allcc as $car)
        <option value="{{ $car->id }}" {{ $car->id == $cc_id ? 'selected' : '' }}>{{ $car->name }}</option>
    @endforeach
</select>
  </div>
...
<div class="form-group">
  ...
  <div class="form-group">
    <label>Edit Service Price </label>
    <input type="number" name="price" id="price" class="form-control w-75" pattern="^[1-9][0-9]*$" required value="{{ $services->price}}"  > 
  </div>
   <div class="form-group">
    <label class="mt-2">Edit Service Slots
     </label>
     <p style="color: grey; font-size: 10px;">Slots are of 30 Minutes Enter Your Number Accordingly, E.g 2=1-hour</p>
    <input type="number" value="{{ $services->hour}}" name="hour" id="hour" pattern="^[1-9][0-9]*$" class="form-control w-75"  required  > 
  </div>
   <div class="form-group">
    <label>Edit Mechanic</label>
     <select name="mechanic" id="mechanic" class="form-control w-75">
   @foreach($allmechanic as $mechanic)
        <option value="{{ $mechanic->id }}" {{ $mechanic->id == $mechanic_id ? 'selected' : '' }}>{{ $mechanic->name }}</option>
    @endforeach
</select>
  </div>
<br>
...
<script > 
  ...
  var servicesData = {!! $allservice->toJson() !!};
