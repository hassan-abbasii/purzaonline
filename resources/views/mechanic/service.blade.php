 ...
 <div class="container-fluid">
               ...
               <div class="d-sm-flex align-items-center justify-content-between  ">
                       <div class="global-flex-new1">
                        <h1 class="h3 mb-0 text-gray-800 head1"><i class="bi bi-tools"></i> All Services</h1>
                         <p style="color: #adb5bd;">(Detail of All Services that mechanics offer in your shop)</p>
                       </div>
     ...
     @if(Session::has('success'))
               ...
               @endif
                        ...
                        <a href=" {{ route('addservice')}}" class=" d-sm-inline-block btn   btn-primary shadow-sm"><i
                                class="bi bi-tools"></i> Add New Service</a>
                    </div>
                     <!-- Content Row -->
...
<div class="aboutdiv-heading-border4"></div>
                    <div class="row car-record justify-content-center">
                      ...
                      <h2>Mechanics Service Record</h2>
            ...
            <table class="table table-striped thead-dark " id="myTable">
               ...
               @foreach($services as $part)
            
       
                <tr>
                  <td>{{ $part->id }}</td>
                  <td>{{ $part->CarCc->name }}</td>
                  <td>{{ $part->mechanicService->service }}</td>
                  <td>{{ $part->mechanicService->service_category }}</td>
                  <td>{{ $part->hour }}</td>
                  <td>{{ $part->price }}</td>
                  <td>{{ $part->shopMechanic->name }}</td>
    ...
    {{ $part->isDeleted ? 'Deleted' : 'false' }}
                  ...
                  <td><div class="global-flex-new justify-content-center"> 
                    <a href="{{ route('editservice', ['id' => $part->id]) }}"><button class="btn btn-secondary btn-sm"><i class="bi bi-pencil-square"></i></button></a>
                    <a href="{{ route('deleteservice', ['id' => $part->id]) }}"><button class="btn btn-secondary btn-sm"><i class="bi bi-trash"></i></button></a></div></td>
